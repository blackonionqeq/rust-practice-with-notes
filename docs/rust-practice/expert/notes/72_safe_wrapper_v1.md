...
safe wrapper 是 unsafe Rust 最常见、也最重要的使用方式：库内部用 unsafe 处理裸指针、FFI、布局或性能细节，对外暴露普通 safe API。

标准库和生态库里很多安全接口都采用这种思路。调用者不需要重复证明底层不变量，因为 wrapper 已经把检查、失败路径和 unsafe 操作封装在一起。
