...
裸指针读取常见于 FFI 返回的只读 buffer、底层容器/迭代器内部从已保存的指针读值、或者库代码在已经完成边界检查后读取内存。

普通业务代码里，优先用 `data.first()`、`data.get(index)`、迭代器或引用。只有当你处理的是“编译器无法用引用表达，但你能证明有效”的地址时，才进入裸指针读取。

## 需要补充看懂的知识

### `as_ptr()` 是什么

`as_ptr()` 通常把 `String`、`str`、`Vec<T>`、slice 等容器里的连续内存暴露成裸指针。

它返回的是 raw pointer 原始指针，例如：

- `str::as_ptr()` 返回 `*const u8`，指向 UTF-8 字节的开头。
- `[T]::as_ptr()` 返回 `*const T`，指向切片第一个元素的位置。
- `Vec<T>::as_ptr()` 返回 `*const T`，指向 Vec 内部缓冲区的开头。

调用 `as_ptr()` 本身是安全的，因为它只是“拿到底层地址”。真正需要 `unsafe` 的是后续通过这个指针读取、写入或做指针偏移。
