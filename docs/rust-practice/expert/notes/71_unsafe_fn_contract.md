...
`unsafe fn` 适合用于“只有调用者知道前提是否成立”的 API，例如调用者传入裸指针、长度必须和指针指向的内存匹配、某段内存必须已经初始化、或外部系统保证了某个生命周期。

如果函数内部可以检查所有前提，就不应该把它写成 `unsafe fn`。这种情况应改成 safe wrapper，把失败路径放进 `Option`、`Result` 或其他普通类型里。
